  ...
  return (
    <div>
      ...
      <div className="px-4 pb-4 pt-4 border-t border-th-border-subtle">
        {/* Status banner — accent when running, red on error, neutral when stopped */}
        {(() => {
        ...
        {enabled && info.url ? (
          (() => {
            ...
            const masked = "•".repeat(token.length || 12);
            ...
            return (
              <>
                ...
                <p className="text-[11px] text-th-text-muted mt-2.5">
                  ...
                  format — drop into Claude, Cursor, Cline, Windsurf, or any MCP
                  client.
                </p>
          ...
          <p className="text-[11px] text-th-text-muted pt-3">
            {enabled
              ? "Couldn't reach the server — toggle it off and on to retry."
              : "Enable the server to get a connection URL."}
          </p>
        )}
      </div>
    </div>

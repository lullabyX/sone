...
import { X, Loader2, ExternalLink, Radio } from "lucide-react";
...
export default function ScrobbleModal({ open, onClose }: ScrobbleModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div
        ref={panelRef}
        className="w-[440px] bg-th-elevated rounded-2xl shadow-2xl max-h-[80vh] flex flex-col overflow-hidden"
        style={{ animation: "slideUp 0.2s ease-out" }}
      >
        {/* Header */}
        <div className="flex items-center gap-3 px-5 pt-5 pb-3">
          <Radio size={20} className="text-th-accent shrink-0" />
          <div className="flex-1 min-w-0">
            <h2 className="text-[16px] font-bold text-th-text-primary leading-tight">
              Scrobbling
            </h2>
          </div>
          <button
        ...
        {/* Footer — pending queue (informational) */}
        {!loading && queueSize > 0 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-th-border-subtle">

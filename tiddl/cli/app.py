...
def callback(
    ...
    jsonl_mode = "--events=jsonl" in sys.argv or (
        "--events" in sys.argv and sys.argv.index("--events") + 1 < len(sys.argv)
        and sys.argv[sys.argv.index("--events") + 1] == "jsonl"
    )
    ctx.obj = ContextObject(
        api_omit_cache=OMIT_CACHE, console=Console(stderr=jsonl_mode), debug_path=debug_path
    )
